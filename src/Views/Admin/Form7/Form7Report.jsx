...
import SingleForm7Detail from './SingleForm7Detail';
import EditForm7Modal from './EditForm7Modal';
...
const Form7Report = () => {
  ...
  const { form7Data } = useContext(DataContext);
  const [form7, setform7] = form7Data;
  const { fetchform7Function } = useContext(DataContext);
  const { fetchform7 } = fetchform7Function;
  const [editModal, setEditModal] = useState(false);
  const [selectedForm7, setselectedForm7] = useState(null);
  ...
  const deleteForm7 = async (form7id) => {
    await axios
      .delete(`${api}/api/form7s/${form7id}`)
      .then((response) => {
        deleteNotification();
        fetchform7();
      })
    ...
    <>
      <div className='flex justify-between items-center mb-2'>
        <p class='text-sm md:text-2xl'>
          {' '}
          खाद्य तथा दाना नमुना विश्लेषण विवरण रिपोर्ट
        </p>
        <div className='flex'>
          <button
            onClick={() => {
            ...
            <>
              <tbody>
                {form7
                  .filter(
                    (form7) =>
                      form7.attributes.timecode >= searchDate1.timestamp1 &&
                      form7.attributes.timecode <= searchDate2.timestamp2
                  )
                  .map((form7) => {
                    return (
                      <tr
                      ...
                      >
                        ...
                        >
                          {form7.attributes.karyalaya}
                        </th>

                        <td class='px-6 py-4'>
                          {form7.attributes.arthikbarsha}
                        </td>
                        <td class='px-6 py-4'>{form7.attributes.date} </td>
                        ...
                        <td class='flex px-6 py-4'>
                          ...
                          >
                            ...
                            >
                              <path
                                fill-rule='evenodd'
                                d='M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm7-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z'
                                clip-rule='evenodd'
                              />
                            </svg>
                          </a>
                          ...
                          {editModal ? (
                            <EditForm7Modal
                              ...
                              form7id={form7.id}
                              attributes={selectedForm7.attributes}
                ...
                {form7.map((form7) => {
                  return (
                    ...
                    >
                      ...
                      <td class='px-6 py-4'>{form7.attributes.arthikbarsha}</td>
                      <td class='px-6 py-4'>{form7.attributes.date} </td>
