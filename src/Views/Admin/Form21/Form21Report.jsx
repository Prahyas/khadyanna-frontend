...
import SingleForm21Detail from './SingleForm21Detail';
import EditForm21Modal from './EditForm21Modal';
...
const Form21Report = () => {
  ...
  const { form21Data } = useContext(DataContext);
  const [form21, setform21] = form21Data;
  const { fetchform21Function } = useContext(DataContext);
  const { fetchform21 } = fetchform21Function;
  const [editModal, setEditModal] = useState(false);
  const [selectedForm21, setselectedForm21] = useState(null);
  ...
  const deleteForm21 = async (form21id) => {
    await axios
      .delete(`${api}/api/form21s/${form21id}`)
      .then((response) => {
        deleteNotification();
        fetchform21();
      })
  ...
  const editRow = (form21) => {
    setselectedForm21(form21);
  };
            ...
            <>
              <tbody>
                {form21
                  .filter(
                    (form21) =>
                      form21.attributes.timecode >= searchDate1.timestamp1 &&
                      form21.attributes.timecode <= searchDate2.timestamp2
                  )
                  .map((form21) => {
                    return (
                      <tr
                      ...
                      >
                        ...
                        >
                          {form21.attributes.karyalaya}
                        </th>

                        <td class='px-6 py-4'>
                          {form21.attributes.arthikbarsha}
                        </td>
                        <td class='px-6 py-4'>{form21.attributes.date} </td>
                        ...
                        <td class='flex px-6 py-4'>
                          ...
                          <a
                            onClick={() => deleteForm21(form21.id)}
                          ...
                          {editModal ? (
                            <EditForm21Modal
                              ...
                              form21id={form21.id}
                              attributes={selectedForm21.attributes}
