...
import SingleForm30Detail from './SingleForm30Detail';
import EditForm30Modal from './EditForm30Modal';
...
const Form30Report = () => {
  ...
  const { form30Data } = useContext(DataContext);
  const [form30, setform30] = form30Data;
  const { fetchform30Function } = useContext(DataContext);
  const { fetchform30 } = fetchform30Function;
  const [editModal, setEditModal] = useState(false);
  const [selectedForm30, setselectedForm30] = useState(null);
  ...
  const deleteForm30 = async (form30id) => {
    await axios
      .delete(`${api}/api/form30s/${form30id}`)
      .then((response) => {
        deleteNotification();
        fetchform30();
      })
  ...
  const editRow = (form30) => {
    setselectedForm30(form30);
  };
  ...
  return (
    <>
      <div className='flex justify-between items-center mb-2'>
        <p class='text-sm md:text-2xl'> मासिक बित्तिय प्रगति रिपोर्ट</p>
        <div className='flex'>
          <button
            onClick={() => {
              setfilterModal(true);
            ...
            <>
              <tbody>
                {form30
                  .filter(
                    (form30) =>
                      form30.attributes.timecode >= searchDate1.timestamp1 &&
                      form30.attributes.timecode <= searchDate2.timestamp2
                  )
                  .map((form30) => {
                    return (
                      <tr
                      ...
                      >
                        ...
                        >
                          {form30.attributes.karyalaya}
                        </th>

                        <td class='px-6 py-4'>
                          {form30.attributes.arthikbarsha}
                        </td>
                        <td class='px-6 py-4'>{form30.attributes.date} </td>
                        ...
                        <td class='flex px-6 py-4'>
                          ...
                          <a
                            onClick={() => deleteForm30(form30.id)}
                          ...
                          >
                            ...
                            >
                              <path
                                fill-rule='evenodd'
                                d='M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zm10 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm10-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z'
                                clip-rule='evenodd'
                              />
                            </svg>
                          </a>
                          ...
                          {editModal ? (
                            <EditForm30Modal
                              ...
                              form30id={form30.id}
                              attributes={selectedForm30.attributes}
