...
import SingleForm11Detail from './SingleForm11Detail';
import EditForm11Modal from './EditForm11Modal';
...
const Form11Report = () => {
  ...
  const { form11Data } = useContext(DataContext);
  const [form11, setform11] = form11Data;
  const { fetchform11Function } = useContext(DataContext);
  const { fetchform11 } = fetchform11Function;
  const [editModal, setEditModal] = useState(false);
  const [selectedForm11, setselectedForm11] = useState(null);
  ...
  const deleteForm11 = async (form11id) => {
    await axios
      .delete(`${api}/api/form11s/${form11id}`)
      .then((response) => {
        deleteNotification();
        fetchform11();
      })
  ...
  const editRow = (form11) => {
    setselectedForm11(form11);
  };
  ...
  return (
    <>
      <div className='flex justify-between items-center mb-2'>
        <p class='text-sm md:text-2xl'> आयात निर्यात गुण प्रमाणिकरण रिपोर्ट</p>
        <div className='flex'>
          <button
            onClick={() => {
              setfilterModal(true);
            ...
            <>
              <tbody>
                {form11
                  .filter(
                    (form11) =>
                      form11.attributes.timecode >= searchDate1.timestamp1 &&
                      form11.attributes.timecode <= searchDate2.timestamp2
                  )
                  .map((form11) => {
                    return (
                      <tr
                      ...
                      >
                        ...
                        >
                          {form11.attributes.karyalaya}
                        </th>

                        <td class='px-6 py-4'>
                          {form11.attributes.arthikbarsha}
                        </td>
                        <td class='px-6 py-4'>{form11.attributes.date} </td>
                        ...
                        <td class='flex px-6 py-4'>
                          ...
                          <a
                            onClick={() => deleteForm11(form11.id)}
                          ...
                          >
                            ...
                            >
                              <path
                                fill-rule='evenodd'
                                d='M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zm11 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm11-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z'
                                clip-rule='evenodd'
                              />
                            </svg>
                          </a>
                          ...
                          {editModal ? (
                            <EditForm11Modal
                              ...
                              form11id={form11.id}
                              attributes={selectedForm11.attributes}
