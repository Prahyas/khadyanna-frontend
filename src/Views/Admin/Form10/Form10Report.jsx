...
import SingleForm10Detail from './SingleForm10Detail';
import EditForm10Modal from './EditForm10Modal';
...
const Form10Report = () => {
  ...
  const { form10Data } = useContext(DataContext);
  const [form10, setform10] = form10Data;
  const { fetchform10Function } = useContext(DataContext);
  const { fetchform10 } = fetchform10Function;
  const [editModal, setEditModal] = useState(false);
  const [selectedForm10, setselectedForm10] = useState(null);
  ...
  const deleteForm10 = async (form10id) => {
    await axios
      .delete(`${api}/api/form10s/${form10id}`)
      .then((response) => {
        deleteNotification();
        fetchform10();
      })
  ...
  const editRow = (form10) => {
    setselectedForm10(form10);
  };
    ...
    <>
      <div className='flex justify-between items-center mb-2'>
        <p class='text-sm md:text-2xl'>
          {' '}
          प्रयोगशाला विश्लेषण प्रतिवेदन सारांश रिपोर्ट
        </p>
        <div className='flex'>
          <button
            onClick={() => {
            ...
            <>
              <tbody>
                {form10
                  .filter(
                    (form10) =>
                      form10.attributes.timecode >= searchDate1.timestamp1 &&
                      form10.attributes.timecode <= searchDate2.timestamp2
                  )
                  .map((form10) => {
                    return (
                      <tr
                      ...
                      >
                        ...
                        >
                          {form10.attributes.karyalaya}
                        </th>

                        <td class='px-6 py-4'>
                          {form10.attributes.arthikbarsha}
                        </td>
                        <td class='px-6 py-4'>{form10.attributes.date} </td>
                        ...
                        <td class='flex px-6 py-4'>
                          ...
                          <a
                            onClick={() => deleteForm10(form10.id)}
                          ...
                          {editModal ? (
                            <EditForm10Modal
                              ...
                              form10id={form10.id}
                              attributes={selectedForm10.attributes}
